...
extension ModelCacheDelegate: CacheDelegate {
  ...
  func modelForKey<T: SimpleModel>(cacheKey: String?, context: Any?, completion: (T?, NSError?) -> ()) {
    guard
      ...
      let jsonObject = cache.objectForKey(cacheKey) as? JSONObject,
      let modelType = T.self as? JSONRepresentable.Type
      else {
        completion(nil, nil) // TODO: Return error as second parameter
        return
    }
    let model = modelType.init(json: jsonObject) as? T
    completion(model, nil)
  }

  func setModel<T: SimpleModel>(model: T, forKey cacheKey: String, context: Any?) {
    if let model = model as? JSONRepresentable {
      cache.setObject(model.asJSON(), forKey: cacheKey)
    } else {
  ...
  func collectionForKey<T: SimpleModel>(cacheKey: String?, context: Any?, completion: ([T]?, NSError?) -> ()) {
    ...
    let collection: [T] = modelIDs.flatMap {
      guard let jsonObject = self.cache.objectForKey($0) as? JSONObject else {
        return nil
      }
      return modelType.init(json: jsonObject) as? T
    }
